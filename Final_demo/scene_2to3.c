...
static void init()
{
    ...
    game_log("2 to 3 scene initialized");
...
static void draw(void)
{
    ...
    al_draw_text(font_pirulen_24, al_map_rgb(0,0,0),400,150,ALLEGRO_ALIGN_CENTER,"Ready for Level 3?");
...
static void on_mouse_down(int btn, int x, int y, int dz)
{
    if (btn == 1)
    {
        ...
        if (pnt_in_rect(x, y, 200, 200, 400, 50))//ready to level 3.
            game_change_scene(scene_level3_create());
...
static void destroy(void)
{
    ...
    game_log("2 to 3 scene destroyed");
}
Scene scene_2to3_create(void)
{
    Scene scene;
