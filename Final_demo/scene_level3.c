...
static ALLEGRO_BITMAP* img_heal_bottle;
static ALLEGRO_BITMAP* img_poison;
static int scores;
...
static MovableObject heal_bottle[MAX_heal];
static MovableObject poison[MAX_poison];
...
static void init(void)
{
    ...
    int i;
    img_background = load_bitmap_resized("Walter Mitty.jpg", SCREEN_W, SCREEN_H);
    img_plane = plane.img = load_bitmap_resized("Anna.png",50,120);
    plane.x = 400;
    plane.y = 500;
    plane.w = 50;
    plane.h = 120;
    img_enemy = load_bitmap_resized("bad_mask.png",75,140);
    ...
    img_enemy2 = load_bitmap_resized("Hans.png",90,100);
    ...
    img_heal_bottle = load_bitmap_resized("heal bottle.png",40,40);
    for (i = 0; i < MAX_heal; i++)
    {
        heal_bottle[i].w = 40;
        heal_bottle[i].h = 40;
        heal_bottle[i].x = heal_bottle[i].w / 2 + (float)rand() / RAND_MAX * (SCREEN_W - heal_bottle[i].w);
        heal_bottle[i].y = heal_bottle[i].h/2 + (float)rand()/RAND_MAX * (SCREEN_H - heal_bottle[i].h);
        heal_bottle[i].vx = 0;
        heal_bottle[i].vy = 1.5;
        heal_bottle[i].img = img_heal_bottle;
    }
    img_poison = load_bitmap_resized("poison.jpg",45,47);
    for (i = 0; i < MAX_poison; i++)
    {
        poison[i].w = 45;
        poison[i].h = 47;
        poison[i].x = poison[i].w / 2 + (float)rand() / RAND_MAX * (SCREEN_W - poison[i].w);
        poison[i].y = poison[i].h/2 + (float)rand()/RAND_MAX * (SCREEN_H - poison[i].h);
        poison[i].vx = 0;
        poison[i].vy = 2;
        poison[i].img = img_poison;
    }
    // [HACKATHON 2-5]
    ...
    //    (vx, vy) to (0, -3).
    // Uncomment and fill in the code below.
    img_bullet = load_bitmap_resized("iceball.png",30,30);
...
static void update(void)
{
    ...
    for(i=0; i<MAX_heal; i++)
    {
        heal_bottle[i].y += heal_bottle[i].vy;
        if(heal_bottle[i].y -heal_bottle[i].h/2 -1.5 >SCREEN_H)
        {
            heal_bottle[i].x = heal_bottle[i].w / 2 + (float)rand() / RAND_MAX * (SCREEN_W - heal_bottle[i].w);
            heal_bottle[i].y = 0;
        }
    ...
    for(i=0; i<MAX_poison; i++)
    {
        poison[i].y += poison[i].vy;
        if(poison[i].y -poison[i].h/2 -1.5 >SCREEN_H)
        {
            poison[i].x = poison[i].w / 2 + (float)rand() / RAND_MAX * (SCREEN_W - poison[i].w);
            poison[i].y = 0;
        }
    ...
    //if we eat HP bottle... it should be consumed and disappear!
    safe_time= al_get_time();
    for(i=0; i<MAX_heal; i++)
    {
        if (plane.x<= heal_bottle[i].x + (heal_bottle[i].w)/2 && plane.x>= heal_bottle[i].x-(heal_bottle[i].w)/2 && plane.y<=heal_bottle[i].y+(heal_bottle[i].h)/2 && plane.y>=heal_bottle[i].y-heal_bottle[i].h/2&& safe_time - last_hitten_time>= 5)
        {
            blood +=10;
            heal_bottle[i].y = -100;
            if(blood >=150)
    ...
    for(i=0; i<MAX_poison; i++)
    {
        if (plane.x<= poison[i].x + (poison[i].w)/2 && plane.x>= poison[i].x-(poison[i].w)/2 && plane.y<=poison[i].y+(poison[i].h)/2 && plane.y>=poison[i].y-poison[i].h/2&& safe_time - last_hitten_time>= 5)
        {
            life = 0;
...
static void draw(void)
{
    ...
    for (i = 0; i < MAX_heal; i++)
        draw_movable_object(heal_bottle[i]);
    for (i = 0; i < MAX_poison; i++)
        draw_movable_object(poison[i]);
    ...
    if(life==0)
        game_change_scene(scene_gameover_create());
    al_draw_textf(font_score, al_map_rgb(0, 0, 0), 150, SCREEN_H-50, ALLEGRO_ALIGN_CENTER,"Scores : %d",scores);
    al_draw_text(font_pirulen_24, al_map_rgb(0, 0, 0), SCREEN_W-200, 15, ALLEGRO_ALIGN_CENTER,"Target : 300");
    al_draw_textf(font_time, al_map_rgb(0, 0, 0), SCREEN_W-300,SCREEN_H-40, ALLEGRO_ALIGN_LEFT,"Time left : %i",(left_time/FPS));
}
...
static void destroy(void)
{
    ...
    stop_bgm(bgm_id);
    game_log("Level 3 scene destroyed");
}
...
Scene scene_level3_create(void)
{
    Scene scene;
