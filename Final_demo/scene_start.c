...
static void init(void) {
	...
	img_plane = plane.img = load_bitmap_resized("player.png",81,81);
	plane.x = 400;
	plane.y = 500;
	plane.w = 81;
	plane.h = 81;
	img_enemy = load_bitmap_resized("pirate.png",51,51);
...
static void update(void) {
    ...
    for(i=0;i<MAX_BULLET_e;i++){
       ...
       safe_time= al_get_time();
        ...
        if(scores>=10)game_change_scene(scene_1to2_create());
}
...
static void draw(void) {
    ...
    if(life==0)game_change_scene(scene_gameover_create());
    al_draw_textf(font_score, al_map_rgb(255, 255, 255), 170, SCREEN_H-50, ALLEGRO_ALIGN_CENTER,"Scores : %d",scores);
    al_draw_text(font_pirulen_24, al_map_rgb(255, 255, 255), SCREEN_W-200, 15, ALLEGRO_ALIGN_CENTER,"Target : 100");
